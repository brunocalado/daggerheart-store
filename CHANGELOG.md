...
- [Added] Prices and tiers for the 280 Hope & Fear items that Daggerheart 2.8 merged into the system compendiums (`daggerheart.weapons`, `armors`, `consumables`, `loot`). Without them the store simply skipped every new item: for default compendiums `store-app.js` only lists a document whose name is a key in the matching `PRICE_DATA` category. Added to `price-data.js` as a marked block at the end of each category — 85 Primary Weapons, 37 Secondary Weapons, 36 Armors, 33 Potions, 31 Consumables, 61 Loot. Prices come from the pricing tables of the `dh-hope-fear` module, which follow the system's equipment bands (tier 1: 10–50 | tier 2: 100–200 | tier 3: 500–1000 | tier 4: 1000–2000), with consumables at roughly a third of the band since they're spent on use. Weapon and armor tiers are taken from the system's own `system.tier`; loot and consumable tiers are judged per item, as the system stores no tier for those two types. Every one of the 635 items across the four system packs now resolves to a price.
- [Added] Both the correct and the misspelled spelling for three items the system compendium ships with a typo in the name — `Soldier's Pike` / `Solider's Pike`, `Advanced Offhand Brass Knuckles` / `Advanced Offhand Brass Knucles`, and `Granminster's Finery` / `Granminister's Finery`. Lookups are by exact name (`getOriginalName`), so registering only one spelling would drop the item from the store either now or the day the system fixes it. Extra keys are inert, the same way the long-standing `Elundrian Chain Mail` alias is.
- [Added] Prices for five items that exist in the 2.8 compendiums but appear in no Hope & Fear pricing table, judged against the same bands and commented inline at each entry: `Concoction` (60/T2), `Sun Tree Sap` (65/T2), `Vial of Moondrip` (20/T1), `Dripfang Poison` (700/T4), and `Brooch` (550/T3 — its description is identical to `Knockback Bracelets`).
- [Changed] `Bladed Star` is listed at tier 3 rather than the tier 1 the system reports. Its `d8+7` damage matches the tier 3 weapons, and the Hope & Fear tier 3 table is where it appears. The store filters and groups by the `PRICE_DATA` tier, so leaving it at tier 1 would have shown a 680-coin weapon in the Tier 1 group. Commented at the entry, and the only tier/price-band conflict in the whole set.
- Verified against Daggerheart 2.8.1 that nothing else needed updating: the pack ids behind `PACK_MAPPING` are unchanged, the Item subtypes behind `VALID_ITEM_TYPES` are unchanged, every `system.*` path the module reads still exists in the 2.8 data models (`armor.max`, `baseThresholds`, `weaponFeatures`, `armorFeatures`, `burden`, `secondary`, `tier`, `attack.roll.trait` / `attack.range` / `attack.damage.main`, `gold.{coins,handfuls,bags,chests}`), and the randomizer needed no change since it reads `PRICE_DATA` at runtime.
